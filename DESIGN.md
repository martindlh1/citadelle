...
**Ce paragraphe a été rouvert à `F2a` et se referme à `F2b`.** *(Rouvert par l'humain, tranché en discussion.)* Annoncer un **trajet** sur une grille où les corps bloquent ne tient pas : le joueur agit après l'annonce, donc le plan est ou bien conditionnel — et l'information cesse d'être complète, ce que le paragraphe ci-dessus refuse en toutes lettres — ou bien rigide, et un ennemi enfermé cogne l'air.
...
**Le grain de ce vocabulaire tient en une ligne.** *(Tranché à `F2b`.)* **Un corps annonce une case exactement quand il peut frapper sans bouger**, et cette annonce **engage** : il frappe cette case quoi qu'il arrive. Sinon il n'annonce que sa nature — *il avance* — et son tour se résout à l'exécution contre le plateau réel.

Ce qu'elle répare est la contradiction qui avait rouvert le paragraphe. Un **trajet** ne s'annonce pas ; une **case** si, mais seulement quand rien de ce que le joueur fera ne peut l'invalider — c'est-à-dire quand le corps n'a pas à se déplacer pour la tenir.

Elle répond **oui** à la question littérale que `F2a` posait, un tireur posté annonce sa case. Mais ce n'est **pas une règle sur les tireurs**, et c'est ce qui écarte l'objection du vocabulaire à géométrie variable : un corps-à-corps déjà collé à un ouvrier annonce la sienne aussi. Ce qui varie est la **situation**, que le joueur lit sur le plateau, jamais la fiche de l'assaillant.

La troisième sortie — n'annoncer **jamais** de case — était la plus courte, et elle tuait une règle entière. On n'esquive pas ce qui n'est pas annoncé : « une intention frappe la case, pas la cible » aurait continué de fonctionner à la résolution sans que le joueur puisse jamais s'en servir, et le déplacement aurait cessé d'être la décision centrale que le paragraphe suivant réclame.

Le troc tient par-dessus, et le harnais le **mesure** : à la manche où la vague est le plus engagée, un simple pas de côté fait tomber les dégâts d'une manche de douze à trois, pour le même nombre de coups portés — la différence est entièrement en coups partis dans le vide. Ce que l'esquive coûte est le tour d'après, où l'assaillant annonce la palissade, qui n'esquive pas.

Le vocabulaire ouvre donc à **deux entrées** — *il frappe cette case*, *il avance*. Le renfort et l'état que ce paragraphe annonce pour plus tard sont `X5` et `X6`, et ils s'y ajouteront sans que rien d'autre bouge.
...
**La borne de tours est un champ de `WaveDef`.** *(Tranché à `F2b`.)* Un siège s'installe là où une escarmouche passe, et la durée d'une razzia est du contenu au même titre que sa composition — les deux vivent dans le même `.tres`, ce qui est la seule façon de les garder d'accord. Elle ne désigne aucun vainqueur : elle dit combien de temps la facture court.

**Ce qu'une razzia emporte se compte par manche passée dans l'enceinte.** *(Tranché à `F2b`.)* Chaque assaillant porte un butin dans son `EnemyData`, et il l'emporte à chaque manche qu'il finit **à l'intérieur du rectangle du bâti**. Le pillage court donc *pendant* la bataille — « ce qu'ils ont cassé et emporté **entre-temps** » — et non à son départ.

Trois choses en découlent, et c'est pourquoi cette lecture a été préférée à « chaque survivant emporte sa part à la fin ». Les deux bonnes façons de jouer paient : les abattre vite, ou les tenir dehors. Une vague bloquée à la lisière repart **les mains vides** sans qu'aucune règle n'ait à le dire. Et la borne ci-dessus cesse d'être une simple durée pour devenir un montant : chaque manche de plus a un prix.

L'enceinte est le rectangle du bâti **tel qu'il était à l'ouverture**, et il ne rétrécit pas quand on rase un mur — le recalculer ferait rapporter *moins* à une vague qui casse *plus*.

**Une vague repartie n'annonce plus rien**, et c'est une règle du domaine plutôt qu'un réflexe d'écran : la borne franchie, ce qui reste affiché serait une menace qui ne tombera jamais. Elle a été trouvée en capture, comme les quatre défauts de `F3a`.
...
La coupure n'était pas arbitraire, et elle s'est vérifiée : tout ce que `F2b` porte supposait de savoir ce qu'un ennemi annonce, ce que le paragraphe des intentions avait rouvert à `F2a`. Écrire un plateau ne l'attendait pas ; écrire une IA ne pouvait pas s'en passer.

*(Écrit à `F2b`.)* **Les deux sont livrés**, et le seul contrat qui compte a bougé exactement comme annoncé : `assault`, `defense` et `breach()` ont quitté le `DamageReport`, `swept()` y est entré. Ce que le combat rend n'est plus une soustraction mais des **faits** — ce qui est tombé, qui est mort, ce qui est parti. `RunOrchestrator.fight()` applique déjà ce rapport sans une ligne à changer, ce qui était toute la promesse de `F1`.
...
**`OUVERT`** — il n'en reste que des **chiffres**. `F2b` a donné à chacune des trois questions qui suivaient `F1` une **place** dans `data/` : la nature des vagues est la liste d'`EnemyData` que porte une `WaveDef`, la borne de tours est un champ de la même `WaveDef`, et le nettoyage complet est **constaté** par le `DamageReport` sans que rien ne le paie. Ce qui reste ouvert est ce qu'on écrit dedans, donc `I3`, et le format n'est plus en cause.
...
- **F2** — Prototype du vrai combat, sur snapshots fabriqués. **Le format n'est plus à définir** : tactique au tour par tour sur la grille du village, deux verbes — se déplacer, attaquer. Voir 3.6, écrit en discussion après `F1`.
  ...
  - **F2b** ✅ — **Ce que les assaillants décident.** `CombatIntent` et `WaveAI` sous `domain/combat/` ; `CombatBoard` qui porte les intentions, la borne, le butin et `to_report()` ; `WaveDef` qui dit enfin **qui vient** et **combien de manches il faut tenir** ; `EnemyData` qui gagne son butin ; `GameDatabase` qui refuse une vague nommant un assaillant inconnu.
    Il a commencé par la question de design que 3.6 gardait ouverte, et la réponse est une seule ligne — **un corps annonce une case exactement quand il peut frapper sans bouger**, et l'annonce engage. Elle répond « oui » à la question littérale sur les tireurs sans être une règle sur les tireurs, ce qui est ce qui la sauve de l'objection du vocabulaire à géométrie variable.
    Il **touche le seul contrat qui compte** : `assault`, `defense` et `breach()` quittent le `DamageReport`, `swept()` y entre, `is_empty()` fond dans `is_held()`. Le rapport cesse de publier une soustraction pour ne dire que des faits, et `RunOrchestrator.fight()` l'applique sans une ligne à changer — ce qui vérifie enfin la promesse que `F1` avait écrite.
    Deux défauts trouvés autrement que par un test, et chacun est de sa famille. **En capture** : la borne franchie, l'écran gardait affichées les quatre cases annoncées à la dernière manche, donc promettait des coups qui ne tomberaient jamais. **Dans une table** : la mesure de l'esquive était prise à la manche 1, où la vague est encore à trois cases de la lisière et n'a personne à portée — un chiffre juste sur la mauvaise manche, donc une table qui ment sans se tromper. Prise là où la vague est le plus engagée, elle dit ce qu'elle devait dire : douze dégâts contre trois, pour le même nombre de coups.
    Il a aussi cassé une suite qu'aucun contrôle ciblé ne regardait — `run_orchestrator_test` fabrique ses propres `WaveDef`, et `RunState.open()` en refuse une sans composition. Un champ de `src/schema/` qu'agrège `RunBalance` voyage aussi loin qu'un champ de `data/balance/`.
- **F3** — Vue de combat, et la fin de journée qui cesse d'être atomique *(cf. 3.8)*. L'applicateur de `RunOrchestrator.fight()` ne bouge pas ; c'est le producteur qui change de nature. Découpé en deux, et **la première moitié est passée avant `F2b`** — c'est l'humain qui l'a demandé, sur un argument que le calendrier d'origine avait manqué : un damier en texte se lit mal, et on n'itère pas sur un format qu'on ne voit pas.
  - **F3a** ✅ — **La bataille, jouée à la souris.** `BodyRenderer` et `CombatPanel` sous `src/adapters/combat/`, le harnais `battle` qui passe du texte à la vue, `TargetHighlight` dont la teinte devient un argument, `BuildingRenderer` qui sait sauter des ancres. Côté domaine, `CombatBoard.strikeable()` — une vue ne juge pas, elle demande — et deux refus qu'un écran rend nécessaires : on ne frappe pas sa propre case, on ne dépense pas son pas pour rester où l'on est.
    **On y joue les deux camps**, et l'ordre s'en trouve meilleur qu'il n'y paraît : `F2b` a remplacé le contrôle du camp d'en face par une IA, donc le même plateau se rejoue des deux façons — la touche `A` rend la vague au joueur. C'était le banc d'essai de la question d'intentions, et il a servi.
  - **F3b** — **L'intégration.** Le producteur branché dans `RunOrchestrator`, la fin de journée qui cesse d'être atomique, `EventBus`. Elle attend `F2b`, qui seul rendra un `DamageReport` à appliquer.
...
**Ordre suivant** — **`F2b`**, qui reprend là où `F2a` s'arrête et commence par trancher le grain des intentions *(cf. 3.6)*. Il se joue désormais **sous les yeux**, `F3a` étant passé devant lui. Puis **`F3b`**, l'intégration, qui n'attend que le rapport de `F2b`. Puis **`M1`**, le menu, que l'écran de fin de `P2a` rend nécessaire en proposant de relancer. Puis **`I3`**, avec la nourriture en tête et son premier chiffre — et maintenant le combat à côté, dont le harnais Bataille dit déjà que la vague est trop dure de beaucoup.

*(Écrit à `F3a`.)* Le déplacement de la vue avant l'IA est la seconde fois qu'un ordre de jalons change en cours de route, après `E1b` passé avant `W1`, et la raison est de la même famille : **on ne fait pas de contenu dans un instrument qu'on ne sait pas lire.** `E1b` déblayait un format de `.tres` avant que douze bâtiments l'écrivent ; `F3a` rend un format de combat regardable avant qu'une IA soit écrite contre lui. Dans les deux cas ce qui bouge est l'ordre, jamais le contenu.
