...
func test_a_tagged_cell_under_the_footprint_counts() -> void:
	...
	assert_int(_inspect(_hut()).count(0)) \
...
func test_the_far_end_of_a_footprint_sees_as_far_as_the_anchor() -> void:
	_grid.set_terrain(ANCHOR + Vector2i(2, 0), _forest)
	assert_int(_inspect(_gallery()).count(0)) \
...
func test_a_zone_that_overruns_the_map_counts_nothing_extra() -> void:
	...
	assert_int(Adjacency.inspect(_hut(), corner, 0, _grid.to_query()).count(0)).is_equal(1)
...
func test_each_cell_pays_what_the_rule_says() -> void:
	...
	assert_int(report.count(0)).is_equal(2)
	assert_int(report.award(0)).is_equal(2)
	assert_dict(report.total()).is_equal({&"wood": 2} as Dictionary[StringName, int])

## **Rien ne borne le produit, et c'est la décision du jalon.** Une cabane au milieu d'un
## bosquet rend neuf fois ce qu'une cabane à un seul arbre rend. Une première version plafonnait
## à deux ; comme source **unique** de production, un plafond aplatit la seule mesure qui
## distingue deux cases, dans un jeu dont le placement est l'essence.
func test_a_full_grove_pays_nine_times_a_single_tree() -> void:
	...
	assert_int(report.count(0)) \
		.override_failure_message("le bosquet ne remplit pas la zone : le cas ne prouve rien") \
		.is_equal(9)
	assert_int(report.award(0)).is_equal(9)

func test_a_rule_that_finds_nothing_pays_nothing() -> void:
	var report := _inspect(_hut())
	assert_int(report.count(0)).is_equal(0)
...
## Deux règles qui versent la même ressource s'additionnent. La case d'eau compte pour les
## deux — rien n'interdit à deux règles de trouver la même case —, et c'est pourquoi la forêt
## n'en rend que huit ici : la neuvième est devenue de l'eau.
func test_two_rules_on_one_resource_add_up() -> void:
	for y in range(ANCHOR.y - 1, ANCHOR.y + 2):
		for x in range(ANCHOR.x - 1, ANCHOR.x + 2):
			_grid.set_terrain(Vector2i(x, y), _forest)
	_grid.set_terrain(ANCHOR + Vector2i(1, 1), _water)
	var data := _hut()
	data.adjacency.append(_rule(&"water", 1, &"wood", 3))
	var report := Adjacency.inspect(data, ANCHOR, 0, _grid.to_query())
	assert_int(report.award(0)).is_equal(8)
...
## Une cabane d'une case : +1 bois par forêt à 1 anneau.
func _hut() -> BuildingData:
	var data := _bare()
	data.adjacency = [_rule(&"forest", 1, &"wood", 1)] as Array[AdjacencyRule]
	return data
